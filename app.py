...
def display_document_embedding_page():
    vector_store_list = [item for item in os.listdir("vector_store") if item != ".DS_Store"]

    st.title("Document Embedding")
    st.markdown("Upload documents to create a custom knowledge base for the chatbot.")

    with st.form("document_input"):
        documents = st.file_uploader("Upload Documents (PDF or TXT)", type=['pdf', 'txt'], accept_multiple_files=True)
    ...
    if save_button:
        if documents:
            ...
            chunks = split_doc(combined_content, chunk_size, chunk_overlap)
            create_new_vs = existing_vector_store == "<New>"

            try:
                embedding_storing(chunks, create_new_vs, existing_vector_store, new_vs_name)
...
def display_chatbot_page():
    ...
    with st.expander("Initialize the LLM Model"):
        st.markdown("""
            Insert the API key and select the vector store, temperature, and maximum character length to initialize the chatbot.
            **NOTE:** Token: API Key from Hugging Face. Temperature: How creative should the chatbot be? (Value between 0 and 1)
        """)

        with st.form("settings"):
            api_key = load_api_key()
            token = st.text_input("Hugging Face Token", type='password', value="******" if api_key else "")
            ...
            initialize_chatbot = st.form_submit_button("Initialize Chatbot")
